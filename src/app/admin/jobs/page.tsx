...
const EMPTY_FORM = {
  slug: '', title: '', department: 'Video Production', type: 'Full-time' as typeof TYPES[number],
  mode: 'Remote' as typeof MODES[number], experience: '', description: '',
  status: 'active' as JobStatus, isUrgent: false, urgentLabel: '',
};
...
export default function AdminJobsPage() {
  ...
  const openEdit = (job: Job) => {
    setEditingJob(job);
    setForm({ slug: job.slug || '', title: job.title, department: job.department, type: job.type, mode: job.mode, experience: job.experience, description: job.description, status: job.status, isUrgent: job.isUrgent, urgentLabel: job.urgentLabel ?? '' });
    setShowModal(true);
  };
                  ...
                  <tr key={job.id}>
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        {job.isUrgent && <span className="adm-urgent"><span className="adm-urgent-dot" />{job.urgentLabel || 'Urgent'}</span>}
                      </div>
      ...
      {deleteTarget && (
        <div className="adm-modal-wrap" onClick={(e) => { if (e.target === e.currentTarget) setDeleteTarget(null); }}>
          <div className="adm-modal" style={{ maxWidth: 420 }}>
            ...
            <div className="adm-modal-body">
              <p className="adm-confirm-text">Are you sure you want to delete <span className="adm-confirm-name">{deleteTarget.title}</span>? This cannot be undone.</p>
            </div>
            <div className="adm-modal-footer">
